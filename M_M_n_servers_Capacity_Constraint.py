...
class sim():
    def __init__(self, lam_bda, mu, queue, num_servers,log:bool) -> None:
        ...
        self.time_depart = [float('inf')] * num_servers #list of departure time for each server
    ...
    def clock(self, max_clock):
        ...
        if self.time <= max_clock: #End simulation if it exceeds the max length of simulation
            if self.time_arrival < min(self.time_depart): #arrival if it is less than minimum of departure list
                self.arrival_event()
            else:
    ...
    def arrival_event(self):
        if self.total_customer_in_system < self.queue + self.num_servers: #Here 1 is for the 1 people being served 
            ...
            if self.busy_servers < self.num_servers: #for the first n customers that arrives and find servers idle
                self.busy_servers += 1
                for i in range(self.num_servers):
                    if self.time_depart[i] == float('inf'):
                        self.time_depart[i] = self.time + self.service_time_gen()
                        break
            self.time_arrival = self.time + self.inter_arrival_gen() #for the customers that arrives and find servers busy
            


            # if self.total_customer_in_system <= self.num_servers: 
            #     self.time_depart = self.time + self.service_time_gen() #for the first arrival
                #arrival time for the next arrival
